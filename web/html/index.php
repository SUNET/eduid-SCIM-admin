...
function showMigrateFlow()
{
  ...
  if ($invite && $invite['status'] == 1) {
    ...
    printf(
      '        <div class="buttons">
    <a class="btn btn-primary" href="?action=activateAccount">%s</a>%s  </div>%s',
      _('Activate account'),
      "\n",
      "\n"
    );
...
function showStartPage()
{
  ...
  print "        <div>";
  printf(
    _('%s uses eduID for logging in to national and international web services.' .
      ' To be able to log in to these, you need to connect your personal eduID identity to your organisation.' .
  ...
  printf(
    ...
    _('When you have completed the above steps, proceed and click one of the buttons below.' .
      ' If you have received an invitation code via email from an administrator, use that option;' .
      ' otherwise, use login via the organisation\’s old login service.'),
...
function showSuccess()
{
  ...
  printf(
    _("Your account is now activated and ready to use when you're logging into services." .
      " Please choose your home organisation (%s) when you're logging in. Please close the window."),
    $config->getScope()
  );
  print '<br>';
  $html->showFooter();
